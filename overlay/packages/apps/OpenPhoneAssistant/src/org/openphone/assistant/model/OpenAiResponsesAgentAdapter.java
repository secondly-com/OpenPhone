...
public final class OpenAiResponsesAgentAdapter implements ModelAdapter {
    ...
    private JSONObject callOrchestratorResponsesApi(String userMessage, boolean hasActiveTask,
        ...
        String prompt = "You are the OpenPhone orchestrator: the single decision point for "
                ...
                + "- clarify: only for a non-actionable conversational question where no phone "
                + "search, observation, or reversible step can reduce the ambiguity. Actionable "
                + "requests must enter act/retrieve/watch/memory instead; the agent loop will "
                + "use phone state and tools before deciding it is blocked. Do not clarify for "
                + "normal preference gaps, random/any/surprise requests, broad exploration, app "
                + "navigation, media playback, search, named contacts, or visible UI choices; "
                ...
                + "free/busy questions. "
                + "A request such as call/text/message a named person is actionable even when "
                + "the user did not provide a phone number. Leave proposed_actions empty and "
                + "set task_goal so the agent can run contacts_search, use the single exact "
                + "match when one exists, and then call the appropriate action tool. Never ask "
                + "whether you may search contacts; searching is the obvious reversible first "
                + "step. "

...
4. Trigger [release.yml](../../.github/workflows/release.yml) from GitHub
   Actions with a version such as `v0.0.1-preview.1`, the device target, the
   release notes markdown file, prerelease status, and whether GitHub should
   mark it as latest.
5. Release validation checks that the release notes file exists, that omitted
   notes map to `docs/releases/<version>.md`, and that the changelog either
   mentions the release or has a clear `Unreleased` section.
